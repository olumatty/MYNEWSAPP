...
const News = ({ countryCode = "" }) => {
    ...
    const fetchNews = async () => {
      ...
      try {
        const res = await fetch(
          `http://api.mediastack.com/v1/news?access_key=${apiKey}&countries=${countryCode}&languages=en&sort=published_desc`
        );
  ...
  return (
    <div>
      <div className="mt-4 ">
        <div className="space-x-3 space-y-3 ml-3 md:space-x-0 md:space-y-0 md:flex mt-3 md:justify-center gap-4">
          {countries.map((country) => (
            <button
              key={country.id}
              className={`min-w-[70px] border rounded-full ${
                activeButton === country.id
                  ? "bg-[#8ecae6] font-semibold text-white"
                  : ""
              }`}
      ...
      <h2 className="mt-3 px-4 mb-8 text-3xl flex justify-center md:block font-bold">
        Headlines for Today
      </h2>
      {loading && <p>Loading...</p>}
